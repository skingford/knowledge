...
export const sections: SectionConfig[] = [
  ...
  },
  {
    ...
    landing: {
      ...
      docs: [
        ...
        { title: 'SQL 优化', href: '/mysql/sql-optimization', description: 'EXPLAIN 分析、索引使用、批量操作与游标分页。' },
    ...
    sidebar: [
      ...
      {
        text: '实战 45 讲：基础篇（01-08）',
        collapsed: true,
        items: [
          { text: '开篇词', link: '/mysql/00-preface' },
          { text: '01 基础架构：SQL 查询如何执行', link: '/mysql/01-sql-query-execution' },
          { text: '02 日志系统：SQL 更新如何执行', link: '/mysql/02-sql-update-log-system' },
          { text: '03 事务隔离：为什么你改了我还看不见', link: '/mysql/03-transaction-isolation' },
          { text: '04 深入浅出索引（上）', link: '/mysql/04-index-part1' },
          { text: '05 深入浅出索引（下）', link: '/mysql/05-index-part2' },
          { text: '06 全局锁和表锁', link: '/mysql/06-global-table-lock' },
          { text: '07 行锁功过', link: '/mysql/07-row-lock' },
          { text: '08 事务隔离还是不隔离', link: '/mysql/08-transaction-isolation-detail' },
        ],
      },
      {
        text: '实战 45 讲：索引篇（09-15）',
        collapsed: true,
        items: [
          { text: '09 普通索引和唯一索引怎么选', link: '/mysql/09-normal-vs-unique-index' },
          { text: '10 为什么会选错索引', link: '/mysql/10-wrong-index-selection' },
          { text: '11 怎么给字符串字段加索引', link: '/mysql/11-string-index' },
          { text: '12 为什么 MySQL 会"抖"一下', link: '/mysql/12-mysql-flush' },
          { text: '13 表数据删一半大小不变', link: '/mysql/13-table-space-reclaim' },
          { text: '14 count(*) 这么慢怎么办', link: '/mysql/14-count-slow' },
          { text: '15 答疑（一）：日志和索引', link: '/mysql/15-qa-log-and-index' },
        ],
      },
      {
        text: '实战 45 讲：查询篇（16-21）',
        collapsed: true,
        items: [
          { text: '16 order by 是怎么工作的', link: '/mysql/16-order-by' },
          { text: '17 如何正确地显示随机消息', link: '/mysql/17-random-message' },
          { text: '18 逻辑相同性能差异巨大', link: '/mysql/18-sql-same-logic-diff-perf' },
          { text: '19 查一行也执行这么慢', link: '/mysql/19-single-row-query-slow' },
          { text: '20 幻读是什么', link: '/mysql/20-phantom-read' },
          { text: '21 改一行锁这么多', link: '/mysql/21-single-row-update-many-locks' },
        ],
      },
      {
        text: '实战 45 讲：高可用篇（22-30）',
        collapsed: true,
        items: [
          { text: '22 饮鸩止渴提高性能', link: '/mysql/22-emergency-perf-boost' },
          { text: '23 怎么保证数据不丢', link: '/mysql/23-data-durability' },
          { text: '24 怎么保证主备一致', link: '/mysql/24-master-slave-consistency' },
          { text: '25 怎么保证高可用', link: '/mysql/25-high-availability' },
          { text: '26 备库延迟好几个小时', link: '/mysql/26-slave-delay' },
          { text: '27 主库出问题从库怎么办', link: '/mysql/27-master-failure' },
          { text: '28 读写分离有哪些坑', link: '/mysql/28-read-write-split-pitfalls' },
          { text: '29 判断数据库是不是出问题了', link: '/mysql/29-database-health-check' },
          { text: '30 答疑（二）：动态观点看加锁', link: '/mysql/30-qa-dynamic-locking' },
        ],
      },
      {
        text: '实战 45 讲：运维与进阶（31-45）',
        collapsed: true,
        items: [
          { text: '31 误删数据后怎么办', link: '/mysql/31-data-recovery' },
          { text: '32 kill 不掉的语句', link: '/mysql/32-unkillable-query' },
          { text: '33 大查询会打爆内存吗', link: '/mysql/33-large-query-memory' },
          { text: '34 可不可以使用 join', link: '/mysql/34-join-usage' },
          { text: '35 join 语句怎么优化', link: '/mysql/35-join-optimization' },
          { text: '36 临时表可以重名', link: '/mysql/36-temp-table-rename' },
          { text: '37 内部临时表', link: '/mysql/37-internal-temp-table' },
          { text: '38 InnoDB vs Memory 引擎', link: '/mysql/38-innodb-vs-memory-engine' },
          { text: '39 自增主键为什么不连续', link: '/mysql/39-auto-increment-gaps' },
          { text: '40 insert 语句的锁', link: '/mysql/40-insert-locks' },
          { text: '41 最快地复制一张表', link: '/mysql/41-fastest-table-copy' },
          { text: '42 grant 与 flush privileges', link: '/mysql/42-grant-flush-privileges' },
          { text: '43 要不要用分区表', link: '/mysql/43-partition-table' },
          { text: '44 答疑（三）：好问题合集', link: '/mysql/44-qa-good-questions' },
          { text: '45 自增 id 用完怎么办', link: '/mysql/45-auto-increment-overflow' },
          { text: '结束语', link: '/mysql/46-conclusion' },
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    landing: {
      ...
      primary: { title: 'Cursor 使用指南', href: '/tools/cursor', description: '整理安装、Tab、Inline Edit、Agent、Rules、MCP、Background Agents、Bugbot 与 CLI 用法。' },
      secondary: { title: 'Gemini CLI 使用指南', href: '/tools/gemini-cli', description: '整理安装、认证、settings.json、GEMINI.md、MCP、skills、subagents 与自动化用法。' },
      scope: [
      ...
      order: ['Cursor 使用指南', 'Gemini CLI 使用指南', 'Codex 使用指南', 'Claude Code 使用指南', 'Vim 实用方案', 'iTerm2 配置指南', 'Git 常用技巧', 'Mac 效率工具'],
    },
    sidebar: [
      {
        text: '工具',
