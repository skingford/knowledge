---
title: Agent 学习综合指南
description: 面向工程师的 Agent 学习主线，覆盖 LLM、Tool Calling、RAG、Workflow、Eval、治理与运行优化。
---
...
- 实验、人工接管与运行优化（PromptOps / HITL / Runtime Optimization）

**通俗理解**：你是在做"大模型驱动的软件系统"，而不只是聊天机器人。

**关键词速览**：`Prompt` · `Context` · `Tool Use` · `RAG` · `Agent Loop` · `Workflow` · `Runtime` · `Evaluation` · `Guardrails` · `Observability` · `HITL`
...
如果你已经开始纠结“这个问题到底该改 Prompt、RAG 还是微调”，继续看 [Fine-tuning 边界与适用场景](./fine-tuning-boundaries.md)。
...
如果你开始在“模型 / 工具 / 知识库到底谁该负责什么”上反复打架，继续看 [模型、工具与知识库协同设计](./model-tool-knowledge-collaboration.md)。
...
- 数据集构建、标注规范、合成数据与失败样本回流
- 长任务异步执行、队列、检查点恢复与幂等
- 灰度发布、变更管理、回滚与发布闸门

### 运行优化

- Prompt 版本化、A/B 实验与回滚
- 人工接管、审批升级、异常转人工
- 缓存、上下文压缩、吞吐和延迟优化
...
如果你准备把评测正式接进工程流程，继续看 [Agent Eval 设计与回归实践](./eval-design-and-regression.md) 会更顺。

如果你已经开始频繁改 Prompt，又担心“改好了还是改坏了”说不清，继续看 [Prompt 版本化、实验与回滚](./prompt-versioning-experiments-and-rollbacks.md)。

如果你发现“大家都在讲效果，但没人能说清样本是怎么来的”，继续看 [Agent 数据集构建、标注与合成数据](./dataset-curation-labeling-and-synthetic-data.md)。

---

## 进阶专题入口

当你已经完成了单 Agent、工具调用、RAG 和基础 workflow 的第一轮落地，接下来最值得补的通常不是继续堆 Prompt，而是把控制面补齐。

| 专题 | 核心问题 | 适合什么时候读 |
|------|----------|----------------|
| [Agent Eval 设计与回归实践](./eval-design-and-regression.md) | 怎么把效果判断从“感觉”变成“回归能力” | 开始频繁改 Prompt / 模型 / 工具配置时 |
| [Agent Memory 设计与状态管理](./memory-design-and-state-management.md) | 什么该记、什么该忘、什么只是任务状态 | 开始做多轮、长期偏好、任务续跑时 |
| [Multi-Agent、Handoff 与路由边界](./multi-agent-handoff-and-routing.md) | 什么时候该拆多个 Agent、handoff 和 tool calling 怎么分 | 单 Agent 已经角色混乱、上下文污染严重时 |
| [Agent 可观测性、回放与故障复盘](./observability-replay-and-incident-review.md) | 线上失败怎么定位、怎么沉淀回归用例 | 准备进入生产、值班和持续优化阶段时 |
| [模型、工具与知识库协同设计](./model-tool-knowledge-collaboration.md) | 模型、工具、知识库、工作流谁该负责什么 | 系统能力越来越多，职责开始打架时 |
| [Fine-tuning 边界与适用场景](./fine-tuning-boundaries.md) | 到底该不该微调，微调前应该先补哪层 | 团队开始频繁讨论微调方案时 |
| [向量数据库选型：pgvector、Milvus、Weaviate、Qdrant](./vector-database-selection.md) | RAG 底座该怎么选，何时该引入专门向量库 | 知识库规模增长、准备正式上 RAG 时 |
| [MCP 协议与工具接入](./mcp-protocol-and-tool-integration.md) | 为什么要标准化外部能力接入，MCP 和 Tool Calling 有何区别 | 工具和资源接入越来越多时 |
| [Guardrails、审批与沙箱边界](./guardrails-approvals-and-sandboxing.md) | 拒答、审批、权限和沙箱各自该怎么控制 | 准备把 Agent 接到真实业务动作时 |
| [模型路由与成本控制](./model-routing-and-cost-control.md) | 强模型 / 快模型如何分层，成本怎么接进回归 | 账单、延迟和模型组合开始成为问题时 |
| [Prompt 版本化、实验与回滚](./prompt-versioning-experiments-and-rollbacks.md) | Prompt、上下文和工具配置怎么做版本管理与灰度回滚 | 团队开始频繁试 Prompt、比模型、做线上实验时 |
| [Human-in-the-loop、人工接管与升级策略](./human-in-the-loop-and-escalation.md) | 什么情况该审批、该转人工、该升级专家处理 | 准备把 Agent 接到真实流程、工单或客户场景时 |
| [Agent 缓存、延迟与吞吐优化](./caching-latency-and-throughput-optimization.md) | 延迟、并发、缓存和吞吐该怎么优化 | 调用量上来、响应慢、预算和 SLA 开始吃紧时 |
| [Agent 数据集构建、标注与合成数据](./dataset-curation-labeling-and-synthetic-data.md) | 真实样本、标注口径、合成扩样和数据版本该怎么做 | 团队开始系统化做 Eval 和回归，却发现样本来源不稳时 |
| [Agent 长任务、队列与检查点恢复](./long-running-agents-queues-and-checkpointing.md) | 多步骤长任务怎么异步执行、恢复和避免重复副作用 | 任务开始跨分钟、跨系统、跨人工环节时 |
| [Agent 灰度发布、变更管理与回滚](./release-strategies-change-management-and-rollbacks.md) | Prompt、模型、工具和知识库怎么整包发布与回滚 | 系统准备进入试点、灰度和持续发布阶段时 |
| [Agent 产品化 Checklist](./agent-productization-checklist.md) | Demo 到试点、灰度、上线前要检查什么 | 准备进入真实用户和真实流量阶段时 |
| [Agent 框架选型对比：LangGraph、LangChain、LlamaIndex、AutoGen、原生 SDK](./framework-selection-and-comparison.md) | 到底该选哪套框架，什么时候该用原生 SDK | 团队准备定技术栈时 |
| [本地知识库构建实践](./local-knowledge-base-practice.md) | 本地文档如何清洗、切块、建索引和评测 | 准备把 Markdown、PDF、笔记接进 RAG 时 |
| [Agent 常见失败案例复盘](./agent-failure-postmortems.md) | 常见事故模式长什么样，应该补哪层 | 团队开始积累失败样本和线上问题时 |
| [Agent 能力自检题与案例](./agent-self-check-and-cases.md) | 自己到底是不是“会做系统”而不只是会讲术语 | 想阶段性自测、面试复盘、带新人时 |
...
6. 学 Workflow / 状态机
7. 补 RAG
8. 补 Eval、Memory、可观测性
9. 补治理、人工接管和成本控制
10. 补 Prompt 实验、缓存、延迟与吞吐优化
11. 补数据集、长任务、灰度发布与回滚
12. 最后再看 Multi-Agent
...
如果你准备把 handoff、路由、上下文传递和权限边界系统化，继续看 [Multi-Agent、Handoff 与路由边界](./multi-agent-handoff-and-routing.md)。
::: warning 注意
...
- 想把评测接进回归流程：读 [Agent Eval 设计与回归实践](./eval-design-and-regression.md)
- 想把记忆和任务状态设计清楚：读 [Agent Memory 设计与状态管理](./memory-design-and-state-management.md)
- 想把多 Agent 路由和 handoff 讲清楚：读 [Multi-Agent、Handoff 与路由边界](./multi-agent-handoff-and-routing.md)
- 想把 Trace、回放和故障复盘做起来：读 [Agent 可观测性、回放与故障复盘](./observability-replay-and-incident-review.md)
- 想把模型、工具和知识库的职责边界讲清楚：读 [模型、工具与知识库协同设计](./model-tool-knowledge-collaboration.md)
- 想判断什么时候该做 Fine-tuning：读 [Fine-tuning 边界与适用场景](./fine-tuning-boundaries.md)
- 想为 RAG 选对存储底座：读 [向量数据库选型：pgvector、Milvus、Weaviate、Qdrant](./vector-database-selection.md)
- 想把工具和资源接入做成标准化层：读 [MCP 协议与工具接入](./mcp-protocol-and-tool-integration.md)
- 想把审批、权限和沙箱边界系统化：读 [Guardrails、审批与沙箱边界](./guardrails-approvals-and-sandboxing.md)
- 想把模型选型、预算和延迟一起纳入设计：读 [模型路由与成本控制](./model-routing-and-cost-control.md)
- 想把 Prompt 调整纳入版本、实验和回滚流程：读 [Prompt 版本化、实验与回滚](./prompt-versioning-experiments-and-rollbacks.md)
- 想把人工审批、转人工和升级链路设计清楚：读 [Human-in-the-loop、人工接管与升级策略](./human-in-the-loop-and-escalation.md)
- 想把缓存、延迟和吞吐优化接进运行时：读 [Agent 缓存、延迟与吞吐优化](./caching-latency-and-throughput-optimization.md)
- 想把真实失败样本沉淀成长期数据资产：读 [Agent 数据集构建、标注与合成数据](./dataset-curation-labeling-and-synthetic-data.md)
- 想把多步骤长任务做成可恢复系统：读 [Agent 长任务、队列与检查点恢复](./long-running-agents-queues-and-checkpointing.md)
- 想把发布、灰度和回滚变成可控流程：读 [Agent 灰度发布、变更管理与回滚](./release-strategies-change-management-and-rollbacks.md)
- 想把 Agent 从 Demo 推到真实上线：读 [Agent 产品化 Checklist](./agent-productization-checklist.md)
- 想为团队选一条更稳的框架路线：读 [Agent 框架选型对比：LangGraph、LangChain、LlamaIndex、AutoGen、原生 SDK](./framework-selection-and-comparison.md)
- 想自己把本地文档做成知识库：读 [本地知识库构建实践](./local-knowledge-base-practice.md)
- 想把线上坑沉淀成复盘资产：读 [Agent 常见失败案例复盘](./agent-failure-postmortems.md)
- 想检查自己是不是真的掌握了这些边界：读 [Agent 能力自检题与案例](./agent-self-check-and-cases.md)
