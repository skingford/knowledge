...
- [Context Engineering 实践要点](#context-engineering-实践要点)
...
## Context Engineering 实践要点

Context Engineering（上下文工程）是 2025 年 Agent 开发中越来越受重视的概念。如果说 Prompt Engineering 关注的是"怎么写指令"，那么 Context Engineering 关注的是**"给模型看什么信息"**。
...
| **上下文排序** | 重要信息放在开头和结尾（模型对这两个位置的注意力最强） |
| **Token 预算管理** | 合理分配上下文窗口给系统指令、检索结果、对话历史 |
| **动态组装** | 根据意图分类动态决定拼入哪些上下文，而非一股脑全塞进去 |
...
| 典型操作 | Few-shot 示例、CoT、角色设定 | 检索策略、上下文裁剪、记忆摘要 |
| 优化目标 | 模型行为的稳定性和准确性 | 信息的相关性和利用效率 |

### 实践技巧

- **上下文裁剪**：RAG 检索结果不要贪多，3-5 个高质量片段比 10 个低质量片段更有效
- **摘要注入**：长对话用摘要替代原始消息，保留关键信息同时节省 token
- **分层上下文**：把"必须遵守的规则"和"参考信息"分开标注，让模型知道哪些可以取舍
- **上下文模板化**：用模板统一拼接格式，避免每次手写导致格式不一致
