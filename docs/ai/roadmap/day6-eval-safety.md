...
## 3.5 输出过滤（Output Filtering）

输入防御可能被绕过，系统提示词加固也不是万无一失。因此，在 Agent 把回答返回给用户之前，还需要做**输出过滤**——这是安全防线的最后一层。

### 需要过滤什么？

- **PII 泄露**：模型可能把邮箱、手机号、身份证号等敏感信息输出给用户
- **系统信息泄露**：模型可能把 system prompt、原始工具返回的 JSON 暴露出去
- **有害内容**：即使有 system prompt 约束，模型仍可能生成不当内容
...
def filter_output(response: str) -> str:
    """
    对 Agent 的输出做最后一道安全过滤。
    在 agent.chat() 返回给用户之前调用。
    """
    filtered = response
    ...
    # 2. 脱敏 PII：手机号（中国大陆格式）
    filtered = re.sub(r'1[3-9]\d{9}', '[手机号已隐藏]', filtered)

    # 3. 移除可能泄露的原始 JSON 工具数据
    # 如果回答中包含大段 JSON，通常不应直接展示给用户
    filtered = re.sub(
        r'\{[^{}]{500,}\}',  # 超过 500 字符的 JSON 块
        '[数据已省略]',
        filtered,
    )

    # 4. 检查禁止词列表
    blocklist = ["system prompt", "系统提示词内容如下", "我的指令是"]
    for phrase in blocklist:
        if phrase.lower() in filtered.lower():
            return "抱歉，我无法提供该信息。请换一个问题。"
...
for text in tests:
    print(f"原始: {text}")
    print(f"过滤: {filter_output(text)}")
    print()
```

### 集成方式

在 Agent 的主循环中，把 `filter_output` 放在返回给用户之前：
...
- 输出过滤是**最后一道防线**，不是唯一防线，要和输入检测、系统提示词加固配合使用
- PII 正则只是基础方案，生产环境建议使用专用的 PII 检测库（如 `presidio`）
- 禁止词列表应根据业务场景定制，并持续更新
- 过滤规则过于激进可能误伤正常回答，需要在安全和可用性之间权衡
...
> **注意**：以下价格截至 2025 年 5 月，LLM 定价变动频繁，使用前请查阅 [OpenAI Pricing](https://openai.com/pricing) 和 [Anthropic Pricing](https://www.anthropic.com/pricing) 获取最新价格。
...
- [OWASP Top 10 for LLM Applications](https://owasp.org/www-project-top-10-for-large-language-model-applications/) — LLM 应用安全的权威清单
- [Promptfoo](https://www.promptfoo.dev/) — 开源 Prompt 评测工具，支持批量测试和版本对比
- [OpenAI: Moderation API](https://platform.openai.com/docs/guides/moderation) — 官方内容安全检测 API
- [Anthropic: Guardrails](https://docs.anthropic.com/en/docs/build-with-claude/guardrails) — Claude 的安全防护实践
