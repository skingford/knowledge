...
把这套双指针套路直接"看进脑子里"：下面用 LeetCode 官方示例 `height = [1,8,6,2,5,4,8,3,7]` 走一遍完整 8 步迭代，重点观察：

- 每一步的宽度、短板、面积是怎么算出来的
- `ans` 什么时候刷新（其实最优解在第 2 步就被锁定）
- 每轮移动的都是短板那一侧，以及为什么这样做不会错过最优解

<InlineSvg src="/images/algorithm/container-with-most-water-two-pointers.svg" alt="盛最多水的容器 双指针动态图例" />

动画里最值得注意的是：第 2 步算出 `area=49` 之后，后面 6 步面积分别是 `18 / 40 / 24 / 6 / 10 / 4`，没有一个能超过 49。这不是巧合——而是"移动长板不可能更优"这条剪枝定理的直接体现。
