...
<InlineSvg src="/images/algorithm/letter-combinations-of-a-phone-number-backtracking.svg" alt="电话号码的字母组合 DFS 回溯动态图例" />

这张图把搜索树和递归状态放到了一起看：

- 左边按 DFS 顺序高亮当前分支，能看到搜索树是怎么一层层往下走的
- 右边同步展示 `dfs(index, path)`、当前动作，以及 `ans` 是如何逐步累积的
- 动画循环一遍后，会更直观地理解“选字符 → 递归下一位 → 回溯撤销”为什么就是回溯模板

如果你更习惯纯文本，再看下面这个简化搜索树：
