...
::: warning 注意
Redlock 依赖时钟同步假设，GC pause 或时钟跳变可能导致多客户端同时持锁。很多核心业务更愿意用数据库约束、状态机或单线程串行化代替”看起来高级”的分布式锁。
:::
...
::: warning 注意
如果业务要求跨全局严格顺序，Kafka 吞吐会明显受限；如果要求单条消息强同步落盘确认，延迟会上升；如果消息体过大、压缩不合理、Partition 过少或过多，吞吐也会明显下降。
:::
...
::: tip 建议
Broker 快不代表 Broker 没瓶颈，磁盘、网络、页缓存、分区数、文件句柄都会成为上限。分区也不是越多越好，分区过多会增加元数据、文件句柄、调度和 rebalance 成本。
:::
...
::: tip 建议
Consumer 慢，很多时候不是 Kafka 拉不下来，而是业务处理太慢。真正的瓶颈往往在 AI 推理、数据库写入、外部 RPC，而不是 Kafka SDK 本身。积压治理不能只盯 MQ，要把下游依赖一起纳入容量设计。
:::
...
::: tip 建议
面试时如果只会背可靠性参数，往往不够。一定要能说出：消息积压本质上是”生产速率超过消费速率”的系统容量问题；排查顺序一般是 Broker 资源、分区分布、消费者 lag、单条消息耗时、下游依赖、重平衡抖动；真正要求强顺序时，往往要接受吞吐下降，并用业务 key 分区、单线程消费或状态机串行化来换一致性。
:::
...
::: warning 注意
失败时优先做快速失败、降级和 fallback，不要无限重试。无限重试会把请求堆死在内存里，最终拖垮整个服务。
:::
...
::: tip 建议
高可用不是”做主从”就结束，而是故障发生时系统如何有序退化。真正重要的是演练、切换脚本、告警门槛和回滚路径。
:::
...
::: warning 注意
AI 服务不能简单照搬普通 Web 服务的资源配置，因为推理负载波动和内存占用波动都更明显。`requests` 压太低会导致节点过度装箱，HPA 只看 CPU 会完全失效。
:::
...
::: danger 警告
绝不裸起 goroutine。每个外部工具调用、模型请求、流式推送协程，都必须绑定派生自顶层请求的 `context.Context`。否则客户端断开后，底层挂起请求无法感知，导致 goroutine 泄漏和内存暴涨。
:::
...
::: danger 警告
涉及资金操作时，Agent 不直接决定交易结果，只能发起受控指令。底层交易系统仍由状态机、幂等、对账和补偿负责兜底。严禁将 LLM 的概率性输出直接传递给核心资金系统。
:::
...
::: danger 警告
涉及打赏扣费、账户余额、清结算时，绝不在应用层做非原子的 `read-modify-write`。先查余额再扣减的写法在并发下会导致超扣。
:::
...
```sql
UPDATE account
SET balance = balance - 100 -- [!code highlight]
WHERE id = 1
  AND balance >= 100; -- [!code highlight]
```
