...
```mermaid
flowchart LR
    A["订单超时任务选型"] --> B{"是否已有成熟 MQ<br/>且希望天然支持分布式？"}
    B -->|是| C["优先延时消息<br/>RocketMQ / RabbitMQ"]
    B -->|否| D{"是否需要自研高吞吐调度层？"}
    D -->|是| E["调度层使用时间轮"]
    D -->|否| F{"规模是否中等<br/>且更看重实现简单？"}
    F -->|是| G["Redis ZSet"]
    F -->|否| H["定时扫表只做兜底"]
    C --> I["数据库条件更新 + 补偿扫描"]
    E --> I
...
flowchart TD
    A["用户下单"] --> B["订单服务"]
    B --> C["写订单表<br/>status=PENDING_PAY<br/>expire_at=now+30m"]
    B --> D["写 Outbox / 发送延时事件"]
    D --> E["延时触发层<br/>RocketMQ / RabbitMQ / Redis / 自研调度器"]
    E --> F["过期消费者"]
    F --> G["可选：获取订单维度分布式锁"]
    G --> H["执行条件更新"]
    H --> I{"更新成功？"}
    I -->|是| J["发布 OrderCancelled 事件"]
    I -->|否| K["结束"]
    J --> L["异步回滚库存 / 退券 / 通知 / 审计"]
    M["补偿调度任务"] --> F
```
