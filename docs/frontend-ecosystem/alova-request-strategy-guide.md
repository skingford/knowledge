---
title: Alova 请求策略指南：缓存、状态管理与跨组件协作
description: 围绕 createAlova、statesHook、useRequest、缓存、自动状态管理、跨组件协作和请求策略，整理一套前端项目中的 Alova 主线。
search: false
---

# Alova 请求策略指南：缓存、状态管理与跨组件协作

Alova 在前端项目里的价值，不只是"另一个请求库"，而是把请求状态和请求策略往上提了一层。
...
Alova 同时支持 Vue、React、Svelte，核心差异只在 `statesHook`——它决定 Alova 如何生成与返回框架对应的响应式状态。
...
以 Vue 为例的完整初始化（React / Svelte 只需替换 statesHook）：
...
export const alova = createAlova({
  ...
  statesHook: VueHook, // React 项目换 ReactHook，Svelte 换 SvelteHook
...
- `statesHook` 决定它如何生成与返回框架响应式状态（Vue 的 ref、React 的 useState、Svelte 的 store）
...
- 如果页面只是偶尔发一次请求，框架自带的状态管理或简单封装已经够用
...
Alova 管的是请求策略和请求状态，全局业务状态仍然是各框架状态管理工具的事：

- Vue 项目里，Pinia 管业务状态，Alova 管请求
- React 项目里，Zustand / Redux 管业务状态，Alova 管请求
- Svelte 项目里，Svelte Store 管业务状态，Alova 管请求

两者的边界：Alova 负责"数据怎么拿、怎么缓存、怎么刷新"，状态管理负责"拿到之后业务怎么用"。混在一起写会让职责模糊，分开反而更清晰。
...
这种时候先把 Axios / 框架封装 / 状态管理边界立住，通常更重要。
