...
| 不想每次都重复输入凭证 | `git config credential.helper store` | 凭证与密码记忆 |
...
| 把最近 N 次提交压成 1 次 | `git reset --soft HEAD~N` | 整理提交历史 |
...
```bash
git config credential.helper store     # 记住账号密码 / Token
```
...
如果你不想每次 push / pull 都重复输入账号密码或 Token，可以先配置凭证助手：
...
第一次成功认证后，Git 会把凭证保存下来，后续同一台机器上再访问相同仓库地址时通常就不会重复提示输入。

::: warning 安全提醒
`store` 会把凭证明文保存在本机文件里，更适合个人电脑或临时环境。  
如果你是 macOS 用户，长期使用更建议考虑系统钥匙串方案，例如 `git config credential.helper osxkeychain`。
:::
...
- 想保留多个提交，但调整顺序、说明或合并方式
- 想把 `fixup` / `squash` 做得更细
- 想删除明显无效的中间提交

#### 常用做法 2：直接压缩最近 N 次提交

如果你只是想把当前功能分支最近的 N 次提交快速压成 1 次，可以直接这样做：

```bash
# 把最近 N 次提交合成 1 个
git reset --soft HEAD~N
git commit -m "feat(branch): summarize feature changes"
git push --force-with-lease
```

如果你习惯一次性提交所有已跟踪文件，下面这种写法也很常见：

```bash
git reset --soft HEAD~N
git commit -am "分支功能内容"
git push --force-with-lease
```

适合这些场景：

- 本地已经提交很多零碎 commit，准备提 PR 前先整理历史
- review 修复做了很多次，想合并成一个更清晰的功能提交
- 分支只由你自己使用，可以安全改写历史

注意这几点：

- `git reset --soft HEAD~N` 会保留改动，并把它们放回暂存区
- `git commit -m ...` 一般就够了；`-am` 更适合你确认改动都来自已跟踪文件时使用
- 推送时优先用 `--force-with-lease`，不要直接用 `--force`
- 不要在多人共用分支、`main`、`release` 这类公共分支上这么做
...
- 想优化终端工作流：读 [iTerm2 配置指南](/tools/iterm2)
- 想提升编辑效率：读 [Vim 实用方案](/tools/vim)
- 想补全本机开发环境：读 [Mac 效率工具](/tools/mac)
