# Golang 高频题对应的源码与原理知识点

## 适合人群

- 想把 Go 高频能力自检题从“会背”提升到“会讲原理”的工程师
- 需要补源码线索和关键实现概念的人
- 准备高级工程师或后端深度能力自检的人

## 学习目标

- 为高频能力自检题补齐源码与底层原理线索
- 知道每个问题应该关注哪些 runtime / sync / net/http 关键词
- 建立“题目 -> 原理 -> 源码入口”的复习方式

## 快速导航

- [适合人群](#适合人群)
- [学习目标](#学习目标)
- [容器与接口](#容器与接口)
- [并发与调度](#并发与调度)
- [内存与 GC](#内存与-gc)
- [同步原语与工程排障](#同步原语与工程排障)
- [网络与服务性能](#网络与服务性能)

## 容器与接口

### Slice 底层结构

- 关键词：`runtime.slice` 思想、切片描述符
- 重点原理：切片不是数组本身，而是对底层数组的一个描述
- 源码关注：扩容、拷贝、共享底层数组带来的数据联动

### Slice 扩容机制

- 关键词：`growslice`
- 重点原理：容量不够时申请新数组并迁移旧数据
- 能力自检延伸：为什么 `append` 后原切片和新切片有时互相影响、有时不影响

### Map 并发不安全

- 关键词：`hmap`
- 重点原理：Map 在写入时可能触发扩容、搬迁 bucket，并发读写会破坏内部状态
- 能力自检延伸：为什么会直接触发 `concurrent map read and map write`

### Map 底层实现

- 关键词：`bucket`、`overflow bucket`
- 重点原理：通过哈希定位 bucket，同一个 bucket 内再找 key
- 能力自检延伸：为什么 Map 查询快，为什么 key 类型会影响性能

### Interface 底层结构

- 关键词：`iface`、`eface`
- 重点原理：接口值包含类型信息和数据地址
- 能力自检延伸：为什么接口调用有动态派发成本

### Nil Interface 问题

- 关键词：动态类型、动态值
- 重点原理：接口是否为 nil，要看“类型”和“值”是否都为空
- 能力自检延伸：为什么 `var err error = (*MyErr)(nil)` 不等于 nil

### 值传递与引用语义

- 关键词：复制语义
- 重点原理：Go 全是值传递，只是某些值里装的是指针
- 能力自检延伸：Slice、Map、Channel 传参为什么看起来像引用传递

## 并发与调度

### Channel 底层原理

- 关键词：`hchan`
- 重点原理：内部有缓冲区、发送队列、接收队列、锁
- 能力自检延伸：无缓冲 Channel 为什么既是通信也是同步

### Channel 阻塞机制

- 关键词：`sendq`、`recvq`
- 重点原理：发送或接收无法立即完成时，Goroutine 会被挂到等待队列
- 能力自检延伸：死锁出现的根本原因是什么

### Goroutine 为什么轻量

- 关键词：小栈、动态扩缩栈
- 重点原理：Goroutine 初始栈很小，由 Runtime 调度，不像线程那样创建成本高
- 能力自检延伸：大量 Goroutine 是否真的“没有成本”

### GMP 调度模型

- 关键词：`G`、`M`、`P`
- 重点原理：P 是执行 Go 代码必须持有的调度资源，M 真正映射系统线程
- 能力自检延伸：为什么不是只有 G 和 M，还要多一个 P

### 工作窃取调度

- 关键词：`work stealing`
- 重点原理：某个 P 的本地队列空了，会去别的 P 偷 Goroutine 来执行
- 能力自检延伸：为什么这样能提升 CPU 利用率

### 抢占式调度

- 关键词：Safe Point、异步抢占
- 重点原理：Runtime 防止某个 Goroutine 长时间霸占 CPU
- 能力自检延伸：老版本和新版本调度体验差异

### Context

- 关键词：链式取消、超时传播
- 重点原理：父 Context 取消后，子 Context 都会收到信号
- 能力自检延伸：为什么 Context 是服务超时治理的核心

## 内存与 GC

### 逃逸分析

- 关键词：编译器分析、`-gcflags=-m`
- 重点原理：变量如果在函数外仍可能被引用，就会分配到堆上
- 能力自检延伸：哪些写法容易触发逃逸

### 栈与堆

- 关键词：生命周期、分配成本
- 重点原理：栈分配快，堆分配会增加 GC 压力
- 能力自检延伸：为什么逃逸不一定是 bug，但常常是优化点

### GC 原理

- 关键词：三色标记、并发标记、清扫
- 重点原理：Go 通过并发 GC 尽量降低 STW 时间
- 能力自检延伸：GC 的目标是低停顿，不只是“回收干净”

### 写屏障

- 关键词：`write barrier`
- 重点原理：并发标记时，需要记录对象引用变化，保证三色不变式
- 能力自检延伸：为什么并发 GC 离不开写屏障

## 同步原语与工程排障

### Defer 执行顺序

- 关键词：后进先出
- 重点原理：`defer` 本质上像压栈，返回前再逐个弹出执行
- 能力自检延伸：多个 `defer` 修改返回值时要怎么分析

### Defer 参数求值

- 关键词：定义时求值
- 重点原理：`defer` 后面的函数参数在注册那一刻就确定了
- 能力自检延伸：为什么闭包 `defer` 和普通 `defer` 表现不同

### Panic / Recover

- 关键词：异常展开、`defer` 链
- 重点原理：`panic` 会沿调用栈向上展开，`recover` 只能在 `defer` 中截获
- 能力自检延伸：为什么 `recover` 放错位置不生效

### sync.Mutex

- 关键词：自旋、阻塞
- 重点原理：锁竞争不激烈时可能先自旋，竞争严重再挂起等待
- 能力自检延伸：为什么锁不是永远越细越好

### RWMutex

- 关键词：读写分离
- 重点原理：允许多个读者并发，但写者需要独占
- 能力自检延伸：为什么读多写少才适合，不然可能更慢

### Atomic

- 关键词：CAS、原子指令
- 重点原理：依赖 CPU 原子操作实现无锁更新
- 能力自检延伸：为什么 Atomic 不能替代所有锁

### WaitGroup

- 关键词：计数器同步
- 重点原理：本质是等待计数归零
- 能力自检延伸：为什么 `Add` 最好在启动 Goroutine 前完成

### sync.Map

- 关键词：Read Map、Dirty Map
- 重点原理：通过读写分层减少锁竞争
- 能力自检延伸：为什么它适合特定场景，不是 Map 的通用替代品

### Pprof

- 关键词：CPU Profile、Heap Profile、Goroutine Profile
- 重点原理：通过采样定位热点函数、分配热点、阻塞热点
- 能力自检延伸：CPU 高、内存高、Goroutine 高，分别该看哪类 Profile

### Goroutine 泄漏

- 关键词：阻塞等待、未退出协程
- 重点原理：Goroutine 不会自动回收，只有执行完才结束
- 能力自检延伸：最常见是 Channel 无人消费、Context 没取消、下游调用卡死

## 网络与服务性能

### net/http

- 关键词：连接复用、Handler、Transport
- 重点原理：服务端和客户端都封装了大量连接管理细节
- 能力自检延伸：为什么不正确复用 `http.Client` 会出问题

### Go 服务性能优化

- 关键词：减少分配、对象复用、降低锁竞争、合理并发
- 重点原理：性能问题最终常落在 CPU、内存、锁、IO、调度这几类
- 能力自检延伸：优化顺序一般是“先测量，再定位，再局部优化”

## 推荐使用方式

- 先看题，再看对应原理关键词
- 再根据关键词回到 Runtime、Sync、Net/HTTP 等源码入口
- 最后补自己的标准回答模板和示例代码
