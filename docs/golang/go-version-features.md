...
var loadData = sync.OnceValues(func() ([]string, error) {
...
这是 Go 诞生以来最重要的语言行为变更之一。在 Go 1.22 之前，`for` 循环的迭代变量在整个循环中共享同一个变量；从 Go 1.22 开始，每次迭代创建独立的变量。这个变更修复了 Go 中最常见的 "陷阱" 之一。
...
func main() {
	...
	fmt.Println("0~99 随机数:", rand.IntN(100))

	// 随机浮点数
	fmt.Println("随机 float64:", rand.Float64())

	// N[T] 泛型函数：适用于任意整数类型
	var n int8 = rand.N[int8](100)
