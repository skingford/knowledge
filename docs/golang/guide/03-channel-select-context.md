...
sequenceDiagram
    ...
    Note over S, R: 无缓冲 Channel · 同步
    S ->> CH: send(data)
    Note over S: 阻塞等待接收者...
    R ->> CH: recv()
    CH -->> R: data
    Note over S, R: 双方必须同时就绪

    Note over S, R: 有缓冲 Channel · 异步
    S ->> CH: send(d1) 写入 buffer
    S ->> CH: send(d2) 写入 buffer
    S ->> CH: send(d3) buffer 满，阻塞
    R ->> CH: recv()
    CH -->> R: d1
    Note over S: buffer 有空位，解除阻塞
```
...
```mermaid
flowchart LR
    Source["数据源"] -->|Fan-out| W1["Worker 1"]
    Source -->|Fan-out| W2["Worker 2"]
    Source -->|Fan-out| W3["Worker 3"]
...
```mermaid
flowchart TD
    BG["context.Background()"] --> T5["WithTimeout · 5s\nmain"]
    T5 -->|"取消信号向下传播"| T2["WithTimeout · 2s\nserviceA"]
    T2 -->|"取消信号向下传播"| SB["serviceB\n继承 2s 超时，3s 任务将被取消"]
```
