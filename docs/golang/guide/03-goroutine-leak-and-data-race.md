...
```mermaid
flowchart TD
    subgraph leak1["场景一 · Blocked Send"]
        G1["Goroutine\nch <- data"] -.->|"无接收者 · 永远阻塞"| FIX1["修复：select + ctx.Done\n或确保有接收者"]
    end

    subgraph leak2["场景二 · Blocked Recv"]
        G2["Goroutine\n<- ch"] -.->|"无发送者 · 永远阻塞"| FIX2["修复：确保 close ch\n或用 ctx 超时控制"]
    end

    subgraph leak3["场景三 · Infinite Loop"]
        G3["Goroutine\nfor ..."] -.->|"无退出条件"| FIX3["修复：select + done\n或 ctx 取消信号"]
    end
```
