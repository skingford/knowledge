...
>
> 图例参考：复用 [连接池、超时、重试与限流](../04-connection-timeout-retry-rate-limit.md) 和 [HTTP 服务端、客户端、TCP/UDP](../04-http-server-client-tcp-udp.md) 里的连接池、超时层次、退避重试图例，先抓治理主线，再回头读 `Transport.RoundTrip`。

这篇专注客户端传输层，不再覆盖 `net/http` 服务端模型和路由细节。

阅读建议：

- 想先理解 `net/http` 全景：看 [net/http 源码精读](./net-http.md)
- 想深入服务端连接生命周期：看 [net/http 服务端高级模式源码精读](./net-http-server.md)

适合带着这些问题来读：

- 为什么 `http.Client` 要复用
- 为什么默认 `MaxIdleConnsPerHost=2` 在生产里常常不够
- 为什么超时要分层配置，而不是只配一个 `client.Timeout`
