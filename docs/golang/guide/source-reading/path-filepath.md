...
>
> 图例参考：这里补了 `path` vs `filepath` 对比图，并复用了 `WalkDir` 递归图和符号链接行为图，先分清“纯字符串路径”和“OS 感知路径”的边界，再回头看 `Clean / EvalSymlinks / WalkDir`。

## 包结构图

<GoNetworkDiagram kind="path-filepath-compare" />
...
<GoNetworkDiagram kind="filepath-symlink" />
