...
>
> 图例参考：复用 [String 与 []byte：转换、构建与数据流最佳实践](../01-string-byte-best-practices.md) 里的 `string/[]byte/[]rune` 和 `strings.Builder` 图，先看“文本语义”和“拼接内存模型”，再回头读 `builder.go` / `buffer.go`。
