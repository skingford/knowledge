...
::: details 点击展开代码：结构体内存对齐与 Offsetof
```go
type Example struct {
...
::: details 点击展开代码：三、指针算术（unsafe.Add）
```go
// Go 1.17+ 引入，编译器直接处理
...
::: details 点击展开代码：四、零拷贝 string ↔ []byte（Go 1.20+）
```go
...
::: details 点击展开代码：五、读取私有字段（反射无法访问时）
```go
...
::: details 点击展开代码：高性能字节处理（避免 string/[]byte 拷贝）
```go
...
::: details 点击展开代码：紧凑结构体设计（减少 padding）
```go
// ❌ 浪费空间（Sizeof = 24）
type Bad struct {
...
::: details 点击展开代码：sync/atomic 操作自定义结构体字段
```go
type Stats struct {
    _       [0]func()  // 防止误用（不可比较）
    count   int64
