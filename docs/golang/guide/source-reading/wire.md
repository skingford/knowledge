...
::: details 点击展开代码：Provider：描述如何创建依赖
```go
...
::: details 点击展开代码：ProviderSet：组合可复用的 Provider 集合
```go
...
::: details 点击展开代码：wire.go：Injector 声明（入口）
```go
//go:build wireinject
// +build wireinject
...
::: details 点击展开代码：wiregen.go：wire 生成的代码
```go
...
::: details 点击展开代码：带清理函数的 Provider
```go
