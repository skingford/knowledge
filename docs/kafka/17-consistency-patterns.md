---
title: "深入拆解消息队列 47 讲：可靠性：分布式集群的数据一致性都有哪些实现方案"
...
不过这里你可能会想，分区写入一定会是均匀的吗？当然不一定。那为什么会不均匀呢？你可以回顾一下 [第09讲](./09-consumer-sdk-part2.md)。 那如何解决倾斜呢？我们会在架构升级篇讲到，你可以先思考下。不过解决这个问题不是我们这节课的重点，这里你只要了解什么是数据倾斜就可以了。
