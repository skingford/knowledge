---
title: "深入拆解消息队列 47 讲：从集群角度拆解 RocketMQ 的架构设计与实现"
...
如上图所示，Dledger 用基于Raft算法实现的Raft Commitlog代替了原来的 Commitlog（Commitlog 请参考 [第11讲](./11-rocketmq-basic-architecture.md)），使得 Commitlog 具备了选举和切换的能力。
...
从设计思路上看，RocketMQ的Controller模式跟我们在 [第16讲](./16-cluster-construction-part2.md) 中提到的独立的Metada Service架构是一致的。如下图所示，如果把图中 Broker 上的Controller上移，就是RocketMQ Controller的实现。
...
异步复制和同步双写就是对应我们在 [第17讲](./17-consistency-patterns.md) 讲到的同步和异步的复制方式， **主要的区别是性能和数据可靠性。**
...
在指标定义记录方面，选用兼容 Promethues 的 Counter、Guage、Histogram 等类型（这三种类型请参考 [第21讲](./21-monitoring-system.md)）来完成指标的记录，并且遵循 Promethues 推荐的指标命名规范。
