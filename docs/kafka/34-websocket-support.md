---
title: "深入拆解消息队列 47 讲：WebSocket：如何在消息队列内核中支持 WebSocket？"
...
这节课我们来看一下如何在消息队列内核中支持 WebSocket。如果你以前了解过 WebSocket，就知道 WebSocket 是一个协议。我们在 [第03讲](./03-protocol-design.md) 中讲过，消息队列在自身私有协议的基础上，还会支持像HTTP这样的公有协议。
...
我们在 [第08讲](./08-consumer-sdk-part1.md) 讲消费模型的时候讲过，Push 模型可以实现当服务端有数据时主动将数据推送给消费者，从而降低消费延时。而客户端要接收数据，一般要启动端口。
...
参考图示，当数据写入到Broker中的Topic tp1 时，因为WebSocket是双向通信的，所以 Broker 收到消息后，可以直接将消息推送给客户端。这个推送的逻辑一般是在内核中维护异步线程去回写数据到客户端实现的。你可以去回看 [第08讲](./08-consumer-sdk-part1.md) 的内容，回顾一下Push模型的实现，结合起来看，理解会更深入一点。
...
就跟我们在 [第03讲](./03-protocol-design.md) 讲到的一样，大家有统一的诉求，但是实际落地就会遇到需求不一致、统一标准的制定周期长、沟通成本高、不够灵活等问题。
