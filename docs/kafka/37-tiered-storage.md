---
title: "深入拆解消息队列 47 讲：云原生：MQ 的分层存储架构都有哪些实现方案？"
description: "极客时间《深入拆解消息队列 47 讲》第 37 讲笔记整理"
---

# 37. 云原生：MQ 的分层存储架构都有哪些实现方案？
...
先来看下图，回顾一下我们在 [第05讲](./05-storage-model.md) 讲过的消息队列底层消息数据的分段存储结构。
...
1. 从CPU的角度，我们可以通过线程绑核操作（参考 [第18讲](./18-java-performance-techniques.md)），在一定程度解决 CPU 隔离的问题。即把上面提到的上传、下载文件的线程绑定到某一批固定的 CPU 核心上，从而让CPU的消耗控制在一定的范围内。
