---
title: "深入拆解消息队列 47 讲：未来：消息队列的技术架构会如何演进？"
...
Serverless 这个词很高大上，简单理解就是 **集群需要具备即时扩缩容的能力**。我们在 [第38讲](./38-serverless-stream-processing.md) 讲过，它核心解决的是 **成本** 问题。
...
上面这句话有点空，没有具体的落地措施，我可以进一步再说说。这里我想表达的是，设计架构时需要保证简洁的理念，不要追求奇技淫巧，追求复杂度和高大上，前面其实讲过挺多了。就比如我们在 [第15讲](./15-cluster-construction-part1.md) 和 [第16讲](./16-cluster-construction-part2.md) 讲集群构建时，集群的元数据有 **基于第三方组件来存储** 和 **集群内部自实现存储** 两种方案，从架构简洁的角度看，我就会建议你使用第二种方案。
