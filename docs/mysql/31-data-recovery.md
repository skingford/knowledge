---
title: "MySQL 实战 45 讲：31. 误删数据后除了跑路，还能怎么办？"
description: "极客时间《MySQL 实战 45 讲》第 31 讲笔记整理"
---
...
<svg viewBox="0 0 520 280" style="width:100%;font-family:system-ui,sans-serif">
  <defs>
    <marker id="dr1-arrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
      <path d="M0,0 L8,3 L0,6" fill="var(--d-blue)"/>
    </marker>
  </defs>
  <!-- Full backup -->
  <rect x="30" y="10" width="160" height="44" rx="6" fill="var(--d-blue-bg)" stroke="var(--d-blue-border)" stroke-width="1.5"/>
  <text x="110" y="30" text-anchor="middle" fill="var(--d-blue)" font-size="12" font-weight="600">全量备份</text>
  <text x="110" y="46" text-anchor="middle" fill="var(--d-text-sub)" font-size="11">（当天 0 点）</text>
  <!-- Binlog backup -->
  <rect x="30" y="80" width="160" height="44" rx="6" fill="var(--d-bg-alt)" stroke="var(--d-border)" stroke-width="1.5"/>
  <text x="110" y="100" text-anchor="middle" fill="var(--d-text)" font-size="12" font-weight="600">Binlog 备份</text>
  <text x="110" y="116" text-anchor="middle" fill="var(--d-text-sub)" font-size="11">（0 点 ~ 12 点）</text>
  <!-- Temp instance -->
  <rect x="310" y="45" width="180" height="44" rx="6" fill="var(--d-bg-alt)" stroke="var(--d-green)" stroke-width="1.5"/>
  <text x="400" y="72" text-anchor="middle" fill="var(--d-green)" font-size="13" font-weight="600">临时实例</text>
  <!-- arrows -->
  <line x1="190" y1="32" x2="307" y2="55" stroke="var(--d-blue)" stroke-width="1.5" marker-end="url(#dr1-arrow)"/>
  <text x="250" y="34" fill="var(--d-text-sub)" font-size="11">恢复</text>
  <line x1="190" y1="102" x2="307" y2="78" stroke="var(--d-blue)" stroke-width="1.5" marker-end="url(#dr1-arrow)"/>
  <text x="250" y="108" fill="var(--d-text-sub)" font-size="11">应用</text>
  <!-- mysqlbinlog step -->
  <rect x="100" y="150" width="320" height="50" rx="5" fill="var(--d-warn-bg)" stroke="var(--d-warn-border)" stroke-width="1"/>
  <text x="260" y="172" text-anchor="middle" fill="var(--d-warn-text)" font-size="12">mysqlbinlog 应用日志，跳过 12 点误操作语句</text>
  <text x="260" y="190" text-anchor="middle" fill="var(--d-warn-text)" font-size="11">（--stop-position / --start-position 或 GTID skip）</text>
  <!-- result -->
  <rect x="130" y="225" width="260" height="38" rx="6" fill="var(--d-blue-bg)" stroke="var(--d-blue-border)" stroke-width="1.5"/>
  <text x="260" y="249" text-anchor="middle" fill="var(--d-blue)" font-size="13" font-weight="600">确认数据 → 恢复回主库</text>
  <line x1="400" y1="89" x2="320" y2="148" stroke="var(--d-blue)" stroke-width="1.2" stroke-dasharray="4,3" marker-end="url(#dr1-arrow)"/>
  <line x1="260" y1="200" x2="260" y2="222" stroke="var(--d-blue)" stroke-width="1.5" marker-end="url(#dr1-arrow)"/>
</svg>
<div style="color:var(--d-text-sub);font-size:0.9em;margin-top:0.3em">图 1 &nbsp;数据恢复流程 — mysqlbinlog 方法</div>
</div>
...
<div style="text-align:center;margin:1.5em auto;max-width:580px">
<svg viewBox="0 0 520 310" style="width:100%;font-family:system-ui,sans-serif">
  <defs>
    <marker id="dr2-arrow" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
      <path d="M0,0 L8,3 L0,6" fill="var(--d-blue)"/>
    </marker>
  </defs>
  <!-- Full backup -->
  <rect x="20" y="10" width="150" height="40" rx="6" fill="var(--d-blue-bg)" stroke="var(--d-blue-border)" stroke-width="1.5"/>
  <text x="95" y="35" text-anchor="middle" fill="var(--d-blue)" font-size="12" font-weight="600">全量备份</text>
  <!-- Temp instance -->
  <rect x="250" y="10" width="150" height="40" rx="6" fill="var(--d-bg-alt)" stroke="var(--d-green)" stroke-width="1.5"/>
  <text x="325" y="35" text-anchor="middle" fill="var(--d-green)" font-size="12" font-weight="600">临时实例</text>
  <line x1="170" y1="30" x2="247" y2="30" stroke="var(--d-blue)" stroke-width="1.5" marker-end="url(#dr2-arrow)"/>
  <text x="208" y="24" fill="var(--d-text-sub)" font-size="10">恢复</text>
  <!-- Online slave -->
  <rect x="20" y="90" width="150" height="55" rx="6" fill="var(--d-bg-alt)" stroke="var(--d-border)" stroke-width="1.5"/>
  <text x="95" y="113" text-anchor="middle" fill="var(--d-text)" font-size="12" font-weight="600">线上备库</text>
  <text x="95" y="132" text-anchor="middle" fill="var(--d-text-sub)" font-size="11">（保留 binlog）</text>
  <!-- Binlog backup -->
  <rect x="20" y="170" width="150" height="40" rx="6" fill="var(--d-bg-alt)" stroke="var(--d-border-dash)" stroke-width="1" stroke-dasharray="4,3"/>
  <text x="95" y="195" text-anchor="middle" fill="var(--d-text-muted)" font-size="11">binlog 备份系统</text>
  <!-- Arrow: temp instance -> slave (as slave) -->
  <line x1="250" y1="50" x2="170" y2="90" stroke="var(--d-blue)" stroke-width="1.5" marker-end="url(#dr2-arrow)"/>
  <text x="230" y="72" fill="var(--d-text-sub)" font-size="10">设为从库</text>
  <!-- Arrow: binlog backup -> slave (dashed) -->
  <line x1="95" y1="170" x2="95" y2="148" stroke="var(--d-text-muted)" stroke-width="1" stroke-dasharray="4,3" marker-end="url(#dr2-arrow)"/>
  <text x="158" y="164" fill="var(--d-text-muted)" font-size="10">补缺失 binlog</text>
  <!-- Filter note -->
  <rect x="250" y="80" width="250" height="55" rx="5" fill="var(--d-warn-bg)" stroke="var(--d-warn-border)" stroke-width="1"/>
  <text x="375" y="100" text-anchor="middle" fill="var(--d-warn-text)" font-size="11" font-weight="600">replicate_do_table 过滤</text>
  <text x="375" y="118" text-anchor="middle" fill="var(--d-warn-text)" font-size="11">只同步误操作的表 + 并行复制加速</text>
  <!-- Result -->
  <rect x="140" y="250" width="240" height="38" rx="6" fill="var(--d-blue-bg)" stroke="var(--d-blue-border)" stroke-width="1.5"/>
  <text x="260" y="274" text-anchor="middle" fill="var(--d-blue)" font-size="13" font-weight="600">确认数据 → 恢复回主库</text>
  <line x1="325" y1="135" x2="300" y2="248" stroke="var(--d-blue)" stroke-width="1.2" stroke-dasharray="4,3" marker-end="url(#dr2-arrow)"/>
</svg>
<div style="color:var(--d-text-sub);font-size:0.9em;margin-top:0.3em">图 2 &nbsp;数据恢复流程 — master-slave 方法（利用并行复制加速）</div>
</div>
...
<td style="padding:6px;border-bottom:1px solid var(--d-border);font-size:12px;font-family:monospace;color:var(--d-text)">begin;<br/>select * from t<br/>where id&gt;1 for update;</td>
...
<td style="padding:6px;border-bottom:1px solid var(--d-border)"></td>
<td style="padding:6px;border-bottom:1px solid var(--d-border);font-size:12px;font-family:monospace"><span style="color:var(--d-orange)">insert into t values(2,2);</span><br/><span style="color:var(--d-orange)">(blocked)</span></td>
</tr>
</tbody>
</table>
<div style="color:var(--d-text-sub);font-size:0.9em;margin-top:0.5em">图 3 &nbsp;复现空表的 next-key lock — insert 被阻塞</div>
</div>
...
<div style="color:var(--d-text-muted);margin-bottom:0.5em">show engine innodb status\G (TRANSACTIONS 部分)</div>
---TRANSACTION 422577, <span style="color:var(--d-green)">ACTIVE</span> 31 sec<br/>
2 lock struct(s), heap size 1136,<br/>
1 row lock(s)<br/>
TABLE LOCK table `test`.`t`<br/>
&nbsp;&nbsp;trx id 422577 lock mode <span style="color:var(--d-orange);font-weight:600">IX</span><br/>
RECORD LOCKS space id 258 page no 3<br/>
&nbsp;&nbsp;n bits 72 index PRIMARY of table `test`.`t`<br/>
&nbsp;&nbsp;trx id 422577 lock_mode X<br/>
&nbsp;&nbsp;<span style="color:var(--d-blue);font-weight:600">lock_type: supremum pseudo-record</span>
</div>
<div style="color:var(--d-text-sub);font-size:0.9em;margin-top:0.5em">图 4 &nbsp;show engine innodb status — 验证 supremum 上的 next-key lock</div>
</div>
