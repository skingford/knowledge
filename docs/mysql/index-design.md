...
很多人把索引理解成“数据库提前排好的一份名单”，这不算错，但还不够精确。更贴近底层的说法是：

> **索引不是一份独立的排序文件，而是 InnoDB 维护出来的一条可寻址、有物理顺序的 B+ 树访问路径。**

把前面 B+ 树的结构再压成 3 个点：

- **非叶子节点**只保存索引键和页指针，像路标，负责把查询导向某个数据页
- **叶子节点**保存完整索引项：聚簇索引叶子节点是整行数据，二级索引叶子节点通常是索引列值 + 主键值
- **叶子节点之间按 key 有序连接**，所以范围扫描、顺序扫描、按索引列排序，本质上都是在顺着这条链表走

这也是为什么下面这类 SQL 往往可以直接利用索引顺序：
...
如果 `id` 上有索引，MySQL 不需要把结果取出来再重新排序，很多时候只要沿着叶子节点链表从右往左扫就行。

一句话记忆：

> **索引擅长的是“顺着既有物理顺序访问”，不擅长“现场临时定义一种新顺序”。**

### 为什么 `CASE WHEN` 排序经常触发 `Using filesort`

问题不在于 `CASE WHEN` “语法高级”，而在于它经常要求一种**索引里原本不存在的逻辑顺序**。

假设某个索引叶子链表上的 `id` 物理顺序是：
...
SELECT *
FROM t
ORDER BY CASE WHEN id IN (1, 2, 5) THEN 0 ELSE 1 END, id;
```

这个 `ORDER BY` 要的逻辑结果其实更接近：
...
- B+ 树里的物理顺序是按原始索引键连好的
- `CASE WHEN` 生成的是运行时才算出来的排序规则
- 数据库没法在不改动叶子链表物理顺序的前提下，直接“跳过 3 和 4 去接上 5”

于是优化器通常只能：

1. 先把候选结果集取出来
2. 把 `CASE WHEN` 的计算结果算出来
3. 放进 `sort_buffer` 里重新排序

这就是 `EXPLAIN` 里经常看到 `Using filesort` 的根本原因。这里的 `filesort` 不一定真的落盘，但它表示：**排序没能直接复用索引顺序，而是额外做了一次排序过程。**

如果业务里真的长期依赖这类“表达式排序”，更稳妥的思路通常不是指望现有索引“自动理解”这个逻辑，而是：
...
#### 1. 区分度（Cardinality）是索引的生命线

在 `SHOW INDEX` 的结果里，`Cardinality` 代表索引唯一值数量的估算值。可以粗略记成：

```text
区分度 = Cardinality / Total Rows
```
...
- `phone`、`email`、`order_no`：通常区分度高，适合做高效过滤
- `status`、`gender`、`is_deleted`：通常区分度低，单独建索引往往收益有限

这里要注意一个细节：**低区分度字段不是永远不能进索引，而是不适合单独做“首列过滤入口”。** 它更常见的用法，是放在高区分度列后面，用于进一步过滤、排序或做覆盖索引。

#### 2. 最左前缀决定“能不能查到”和“能不能免排序”
...
你可以把它理解成一本字典：先按 `a` 排，同一个 `a` 里再按 `b` 排，再按 `c` 排。
...
-- ✅ a 确定后，b 天然有序，常有机会消除排序
SELECT * FROM t WHERE a = 1 ORDER BY b;
```

所以最左前缀不只是回答“这条 SQL 能不能走索引”，还回答“这条 SQL 能不能顺便把排序也做了”。

#### 3. 覆盖索引尽量让查询在一棵树里结束

覆盖索引是很重要的一条优化思路：**如果索引里已经包含查询需要的全部列，就没必要回表。**

对比一下：

```sql
-- 只有 idx_status(status)
SELECT name FROM user WHERE status = 1;
```

这类查询通常会先在 `idx_status` 上找到主键，再回聚簇索引把 `name` 取出来。
...
那么下面这条 SQL 就更容易直接在索引里完成：

```sql
SELECT name FROM user WHERE status = 1;
```

虽然 `status` 本身区分度不高，但如果这是高频查询，并且返回列很少，覆盖索引仍然可能有明显收益。它优化的不是“过滤多准”，而是“少回多少次表”。
...
- 聚簇索引按主键组织整张表
- 二级索引叶子节点存的也是主键值
- 主键一旦又长又随机，整棵树都会变胖，插入也更容易打散页内顺序
...
这样的主键设计。它既减少页分裂，也让二级索引更轻。

#### 5. 索引不是越多越好，要把维护成本一起算进去

索引带来查询收益的同时，也一定会带来维护成本：

- **空间成本**：每个索引本质上都是一棵额外的 B+ 树，要占磁盘，也会吃 Buffer Pool
- **写入成本**：`INSERT`、`UPDATE`、`DELETE` 都要同步维护索引，可能触发页分裂、页合并、记录搬移
- **优化器成本**：候选索引越多，优化器选择路径越复杂，统计信息一旦不准，还可能选错索引

所以“给慢 SQL 加索引”不是终点，更完整的问题应该是：

> **这条索引带来的读收益，是否足以覆盖它长期的写成本和维护成本？**

每次看慢 SQL，先问自己五个问题：

1. `WHERE` 条件命中最左前缀了吗？
2. `ORDER BY` / `GROUP BY` 能不能直接复用索引顺序？
