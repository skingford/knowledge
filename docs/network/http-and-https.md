...
如果你已经能讲清 HTTP 版本差异，但还容易在证书、SNI、mTLS 或握手失败上卡住，建议继续读：

- [TLS、证书与 mTLS](./tls-and-certificates.md)：专门拆开证书链、SNI、ALPN、mTLS 和常见 `x509` / 握手错误
- [Socket、端口与连接状态](./socket-and-connection-state.md)：把 `connection refused`、RST、TIME_WAIT、CLOSE_WAIT 放回连接生命周期里理解
- [代理、反向代理、网关与隧道](./proxy-gateway-and-tunnel.md)：把反向代理、TLS 终止、Upgrade 和常见 `502` / `504` 放回入口链路里理解
- [网络排障与常用命令](./troubleshooting-and-commands.md)：把 `curl -v`、`openssl s_client`、`ss`、`tcpdump` 串成一套排障动作

## 延伸阅读

- 想补传输层协议：读 [TCP 与 UDP](./tcp-and-udp.md)
- 想专门补 TLS、证书和 mTLS：读 [TLS、证书与 mTLS](./tls-and-certificates.md)
