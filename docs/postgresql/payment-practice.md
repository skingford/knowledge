...
支付场景里最容易答浅的，通常不是概念本身，而是没有把“长事务为什么危险、流水表为什么适合分区、复制延迟为什么会影响查询一致性”讲成一条业务链。下面三张 SVG 图例先把这三条线立起来。

### 1. 长事务为什么危险

<PostgreSQLPaymentDiagram kind="long-transaction-pressure" />

### 2. 流水表为什么适合分区

<PostgreSQLPaymentDiagram kind="partition-archive-flow" />

### 3. 复制延迟为什么会影响读一致性

<PostgreSQLPaymentDiagram kind="replica-lag-read-path" />
