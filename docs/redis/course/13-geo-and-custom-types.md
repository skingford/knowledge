...
<RedisCourseFigure kind="geo-hash-approach" />
...
<RedisCourseFigure kind="geo-sortedset-approach" />
...
<RedisCourseFigure kind="geo-longitude-encoding" />

对纬度的编码方式，和对经度的一样，只是纬度的范围是[-90，90]，下面这张表显示了对纬度值 39.86 的编码过程。

<RedisCourseFigure kind="geo-latitude-encoding" />
...
<RedisCourseFigure kind="geo-interleave-encoding" />
...
<RedisCourseFigure kind="geo-grid-mapping" />
...
<RedisCourseFigure kind="geo-edge-case" />
...
<RedisCourseFigure kind="geo-redisobject-ptr" />
...
<RedisCourseFigure kind="geo-newtype-steps" />

**第一步：定义新数据类型的底层结构**

我们用 newtype.h 文件来保存这个新类型的定义，具体定义的代码如下所示：

::: details 点击展开 `NewTypeObject` 结构定义
...
::: details 点击展开 `createNewTypeObject` 示例
...
::: details 点击展开 `ntinsertCommand` 实现
