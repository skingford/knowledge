...
<RedisCourseFigure kind="failover-repl-timeout" />
...
<RedisCourseFigure kind="failover-repl-storm" />

为了方便你理解，我给你举两个例子。

第一个例子是使用 EXPIRE 命令，当执行下面的命令时，我们就把 testkey 的过期时间设置为 60s 后。

::: details 点击展开 `EXPIRE` 示例
```
EXPIRE testkey 60
```
:::
...
<RedisCourseFigure kind="failover-cascade-fix" />
