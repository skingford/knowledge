---
title: "Redis 核心技术与实战：38. 通信开销：限制Redis Cluster规模的关键因素"
description: "极客时间《Redis 核心技术与实战》38. 通信开销：限制Redis Cluster规模的关键因素 笔记整理"
---
...
```c
typedefstruct{
charnodename[CLUSTER_NAMELEN];//40字节
uint32_tping_sent;//4字节
uint32_tpong_received;//4字节
charip[NET_IP_STR_LEN];//46字节
uint16_tport;//2字节
uint16_tcport;//2字节
uint16_tflags;//2字节
uint32_tnotused1;//4字节
