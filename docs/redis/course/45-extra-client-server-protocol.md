---
title: "Redis 核心技术与实战：加餐（四）：Redis客户端如何与服务器端交换命令和数据？"
description: "极客时间《Redis 核心技术与实战》加餐（四）：Redis客户端如何与服务器端交换命令和数据？ 笔记整理"
---
...
::: details 点击展开 `HSET` 交互示例

```bash
#成功写入Hash类型数据，返回实际写入的集合元素个数
127.0.0.1:6379>HSET testhash a1b2c3
(integer)3
```
...
(error) ERR unknown command'PUT',withargs beginningwith:'testkey','testvalue'
```
...
<RedisCourseFigure kind="extra-resp-protocol" />
...
```bash
-ERR unknown command`PUT`,withargs beginningwith:`testkey`,`testvalue`
```
...
<RedisCourseFigure kind="extra-pipeline-batch" />
...
::: details 点击展开 `HGETALL / ZRANGE` 返回示例
...
5)"c"
6)"3"
127.0.0.1:6379>ZRANGE testzset03withscores
1)"a"
2)"1"
3)"b"
4)"2"
